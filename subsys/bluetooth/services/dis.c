 ...
 */
static uint8_t dis_system_id[8] = {
	BT_BYTES_LIST_BE24(CONFIG_BT_DIS_SYSTEM_ID_OUI),
	BT_BYTES_LIST_BE40((uint64_t)CONFIG_BT_DIS_SYSTEM_ID_IDENTIFIER)};
#endif
 ...
 */
static void read_udi_subval(const char *str, uint16_t str_len, char *buf, uint16_t *bytes_read,
			...
			uint16_t len, uint16_t offset)
{
	...
	read_udi_subval(BT_DIS_UDI_DI_STR_REF, strlen(BT_DIS_UDI_DI_STR_REF), buf_i, &bytes_read,
			&index, len, offset);
	read_udi_subval(BT_DIS_UDI_ISSUER_STR_REF, strlen(BT_DIS_UDI_ISSUER_STR_REF), buf_i,
			&bytes_read, &index, len, offset);
	read_udi_subval(BT_DIS_UDI_AUTHORITY_STR_REF, strlen(BT_DIS_UDI_AUTHORITY_STR_REF), buf_i,
			&bytes_read, &index, len, offset);
...
static int dis_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *store)
{
	...
	if (!strncmp(name, "sysid_oui", nlen)) {
		...
		if (len < 0) {
			...
			sys_put_be24(oui, &dis_system_id[0]);
	...
	if (!strncmp(name, "sysid_identifier", nlen)) {
		...
		if (len < 0) {
			...
			sys_put_be40(identifier, &dis_system_id[3]);
